...
public class Polynomial extends ArrayList<Integer> {
    ...
    public String toString() {
        ...
        if (deg == 0)
            return "" + this.get(0);
        if (deg == 1 && this.get(0) != 0) {
            if (this.get(1) != 1 && this.get(1) != -1) {
                if (this.get(0) >= 0)
                    return this.get(1) + "x + " + this.get(0);
        ...
        if (this.get(deg) == 1)
            s = "x^" + deg;
        else if (this.get(deg) == -1)
            s = "-x^" + deg;
        else
            s = this.get(deg) + "x^" + deg;
    ...
    public static int degreeCoeffsHighToLow(int[] coeffsHighToLow) {
        ...
        int count = coeffsHighToLow.length - answer - 1;
        return count;
    }
    ...
    public static int[] lowToHigh(int[] coeffsHighToLow) {
        ...
        int[] answer = new int[coeffsHighToLow.length - count];
        for (int i = coeffsHighToLow.length - 1; i >= count; i--) {
            answer[coeffsHighToLow.length - 1 - i] = coeffsHighToLow[i];
        }
        return answer;
    }
    ...
    public static int[] highToLow(int[] coeffsLowToHigh) {
        ...
        int[] answer = new int[coeffsLowToHigh.length - count];
        for (int i = 0; i < coeffsLowToHigh.length - count; i++) {
            answer[coeffsLowToHigh.length - count - 1 - i] = coeffsLowToHigh[i];
        }
        return answer;
    }
    ...
    public Polynomial plus(Polynomial p) {
        int size_p = p.getDegree() + 1;
        int size_this = this.getDegree() + 1;
        int[] arr;
        if (size_p >= size_this) {
            arr = new int[size_p];
            ...
            for (int j = 0; j < size_this; j++) {
                arr[j] = arr[j] + this.get(j);
            }
        } else {
            arr = new int[size_this];
        ...
        int[] used = Polynomial.highToLow(arr);
        Polynomial ans = new Polynomial(used);
        return ans;
    }
    ...
    public Polynomial times(Polynomial p) {
        int[] result = new int[this.size() + p.size() - 1];
        for (int i = 0; i < this.size(); i++) {
            for (int j = 0; j < p.size(); j++) {
                result[i + j] += (this.get(i) * p.get(j));
            }
        }
        result = Polynomial.highToLow(result);
    ...
    public Polynomial minus(Polynomial p) {
        int[] coe = new int[]{-1};
        Polynomial times_func = new Polynomial(coe);
        Polynomial after_times = p.times(times_func);
        Polynomial ans = this.plus(after_times);
        return ans;
